...
- Main process: `app/src/main.ts` — app boot, `BrowserWindow` management, `ipcMain` handlers.
- Preload bridge: `app/src/preload.ts` — exposes `window.electronAPI` to renderer (use these methods from frontend).
- Renderer (UI): `app/src/index.tsx` and `app/src/frontend/**` — React + TanStack Router routes are generated into `app/src/routeTree.gen.ts` (do not edit).
- Backend helpers (file I/O, thumbnails, project logic): `app/src/backend/*.ts` called by the main process.

## Key files and patterns

- IPC event constants: `app/src/constants.ts` defines `IPC_EVENTS`. Use these names when linking main/preload/renderer behaviour.
- Preload API: call `window.electronAPI.*` from React. Example: `await window.electronAPI.savePhotoFile(data, arrayBuffer)` — handled in `app/src/main.ts` via `ipcMain.handle(IPC_EVENTS.SAVE_PHOTO_FILE, ...)`.
- Edit window flow: the main process opens edit windows by encoding JSON with `btoa` into the URL query (`?data=${btoa(JSON.stringify(data))}`) in `app/src/main.ts`; the edit route decodes with `atob` in `app/src/routes/edit.tsx`.
- Generated routes: `app/src/routeTree.gen.ts` is autogenerated by TanStack Router — do not modify it directly; edit files in `app/src/routes/` instead.
- Types: central types live in `app/src/types.ts` — use these for consistent shapes between main and renderer. Project-file types (`PhotoEdits`, `PhotoBody`, `CollectionBody`, `MatchedBody`, `ProjectBody`) are derived from Zod schemas in `app/src/schemas.ts` via `z.infer`.
- Schema validation: `app/src/schemas.ts` defines Zod schemas for project data. Project files are validated against these schemas on load via `parseProjectFile` in `app/src/backend/projects.ts`.

## Repository Structure

- `app/src/main.ts` — Main process entry point, window management, IPC handlers
- `app/src/preload.ts` — Preload script exposing `window.electronAPI` to renderer
- `app/src/index.tsx` — React app entry point
- `app/src/constants.ts` — Centralized constants and IPC event definitions
- `app/src/types.ts` — Central types for consistent data shapes between main and renderer
- `app/src/schemas.ts` — Zod schemas for project-file data, some types in `app/src/types.ts` are derived from these
- `app/src/helpers.ts` — Utility helpers for common operations
- `app/src/backend/` — Backend file I/O and image processing:
...
- `app/src/frontend/` — React UI components and hooks:
  ...
  - `hooks/` — Custom React hooks for UI logic:
    ...
    - `imageEditor/` — Feature-focused hooks used by `useImageEditor`:
      ...
      - `useImageFilters.ts` — Manage filter state (`ImageFilters` in `app/src/types.ts`) for brightness, contrast, saturation, and edge detection
      - `useImageTransform.ts` — Manage zoom/pan transform state (`ImageTransformations` in `app/src/types.ts`) and coordinate conversions
...
- `app/src/routes/` — TanStack Router route definitions (router generates `app/src/routeTree.gen.ts`)
- `app/src/models/` — Data model classes:
  - `Project.ts` — Project model
  - `Photo.ts` — Photo model
  - `Collection.ts` — Collection model
- `app/src/contexts/` — React Context providers (e.g., ProjectContext)
- `docs/` — User and technical documentation
- Tests: `*.test.ts` files co-located with source files
- Configuration: `app/tsconfig.json`, `app/vite.*.mts`, `app/forge.config.ts`, `app/vitest.config.ts`
