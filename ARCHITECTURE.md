...
- Edit window flow: the main process opens edit windows by encoding JSON with `encodeEditPayload` (JSON stringify then base64 encode via `Buffer.from().toString('base64')`) into the URL query (`?data=<base64>`) in `src/backend/ipc/editorHandlers.ts`. The edit route decodes with `decodeEditPayload` (base64 decode then Zod parse) in `src/routes/edit.tsx`. Both helpers are in `src/helpers.ts`.
