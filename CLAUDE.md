...
- ALWAYS read `docs/` (user-facing documentation, published via MkDocs) for further context. The contents of `docs/` MUST be kept in sync with any user-visible change in the same PR — same rule as above. Concretely:
  - `docs/user-guide/usage.md` — update for any change to project/editor/export workflow, UI affordances, or settings.
  - `docs/user-guide/quickstart.md` — update if the five-step happy path changes (folder → stacks → matched pair → optional edit → export).
...
- `CONTRIBUTING.md` should be updated when contributor setup, testing, or PR conventions change.
- `CHANGELOG.md` is generated by Changesets — do not edit by hand; add a changeset via `npm run changesets:add` instead.
...
- Sentry: integrated in both main (`@sentry/electron`) and renderer (`@sentry/electron/renderer`). Opt-in via settings; when enabled, captures crashes, errors, session replay (error-only), tracing, profiling, and console logs. Initialised before app ready; user preference applied in `whenReady`. Privacy policy at `docs/privacy.md`. Environment variables: `VITE_SENTRY_DSN` (renderer) and `SENTRY_DSN` (main). See `.env.example`. **Telemetry is always opt-in**: `DEFAULT_SETTINGS.telemetry` MUST stay `"disabled"`, and no code path may send telemetry without the user first enabling it in settings. This is a documented commitment to users in `docs/privacy.md` — do not change the default or introduce opt-out behaviour without first updating that page.
...
- **Don't call `save()` from call sites** — mutating methods on `Collection` and `Project` call `Project.save()` internally. Calling `save()` again from the call site is redundant. The save is debounced (see `SAVE_PROJECT_DEBOUNCE_MS` in `src/constants.ts`), so multiple mutations in the same tick — e.g. a photo move that removes from one collection and adds to another — coalesce into a single write, and no intermediate state hits disk.
