...
On macOS and Windows, `app.close()` shuts down Electron gracefully via CDP. On Linux under `xvfb-run`, the Electron process stops responding to Playwright's CDP protocol after the tests complete, causing `app.close()` to hang indefinitely. The `afterAll` hook therefore uses a platform-conditional approach: `app.close()` on macOS/Windows, and process group kill on Linux.

Playwright spawns Electron with `detached: true` on Linux, making it a process group leader. The kill must target the entire group rather than just the main PID - renderer child processes inherit `stdio` pipes and keep the CDP WebSocket alive. This is the same approach recommended by the Playwright team ([#39248](https://github.com/microsoft/playwright/issues/39248), [#20016](https://github.com/microsoft/playwright/issues/20016)).
