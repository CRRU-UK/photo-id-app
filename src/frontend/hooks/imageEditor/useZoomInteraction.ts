...
export const useZoomInteraction = ({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      ...
      const isPixelScroll = event.deltaMode === WheelEvent.DOM_DELTA_PIXEL;
      const wheelFactor = isPixelScroll ? ZOOM_FACTORS.WHEEL_PIXEL : ZOOM_FACTORS.WHEEL_LINE;
      const delta = event.deltaY > 0 ? 1 / wheelFactor : wheelFactor;
